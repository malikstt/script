...
-- Cached FPS tracking: use a single counter, avoid per-frame string concat until needed
pcall(function()
	local frameCount = 0
	local lastTime = tick()
...
-- Ping loop: cache Stats.Network lookup outside loop
task.spawn(function()
...
task.spawn(function()
	...
	local ok, err = pcall(function()
		...
		task.spawn(function()
			local ok1, ok2
			repeat
				ok1, upgradeServiceClient_new = pcall(function() return require(ReplicatedStorage.Source.Features.Upgrades.UpgradeServiceClient) end)
				ok2, dataServiceClient_new = pcall(function() return require(ReplicatedStorage.Packages.DataService).client end)
...
local function getEnemyScore(enemy, rootPos)
	...
	local diff = root.Position - rootPos
	local distSq = diff.X*diff.X + diff.Y*diff.Y + diff.Z*diff.Z
	if distSq > RANGE_SQ then return nil end
...
local function computeScores(rootPos, boundary)
	local entries = {}
	local cachedLen = #cachedEnemies
	for i = 1, cachedLen do
		local enemy = cachedEnemies[i]
	...
	local priCoins   = pri["Most Coins & Goop"]
	local priClose   = pri["Closest"]
	local priLowHP   = pri["Lowest HP"]
	local priMut     = pri["Mutations Only"]
	for i = 1, #entries do
		local e = entries[i]
		local s = 0
		if priCoins   then s = s + ((maxCoins > 0 and e.coins/maxCoins or 0) + (maxGoop > 0 and e.goop/maxGoop or 0)) / 2 end
		if priClose   then s = s + (maxDist > 0 and 1 - e.dist/maxDist or 0) end
		if priLowHP   then s = s + (maxHealth > 0 and 1 - e.health/maxHealth or 0) end
		if priMut     then s = s + (getMutation(e.enemy) and 1 or 0) end
		scores[e.enemy] = s
	end
...
-- Heartbeat: throttle autoFarm logic to ~10hz instead of every frame
local lastAutoFarmTick = 0
RunService.Heartbeat:Connect(function()
	local now = tick()
	if now - lastAutoFarmTick < 0.1 then return end
	lastAutoFarmTick = now
	pcall(function()
		refreshEnemyCache()
	...
	if selectedSlimeMode == "Best" then
		...
		for _, slimeKey in ipairs(equipped) do
			if type(slimeKey) == "string" and slimeKey:sub(1,1) == "." then
				local inv = dataServiceClient:get("inventory") or {}
				local data = inv[slimeKey]
				if type(data) == "table" then result[#result + 1] = {key=slimeKey, data=data} end
			end
		end
		return result
	end
...
local function resolveFruitList()
	...
	if selectedFruitIds[1] == "ANY" then
		local result = {}
		for _, f in ipairs(ALL_FRUITS) do if owned[f.id] then result[#result + 1] = f.id end end
		return result
	else
		local result = {}
		for _, fid in ipairs(selectedFruitIds) do if owned[fid] then result[#result + 1] = fid end end
		return result
	end
end
...
-- Auto feed: throttled to 1hz instead of every Heartbeat frame
local lastFeedTick = 0
featureToggle(farmingTab, {
	...
	Callback = function(value)
		...
		if value then
			feedConnection = RunService.Heartbeat:Connect(function()
				...
				local now = tick()
				if now - lastFeedTick < 1 then return end
				lastFeedTick = now
				pcall(doFeed)
			end)
		end
	...
	farmingTab:CreateDropdown({
		...
		Callback = function(options)
			...
			for _, label in ipairs(picked) do
				...
				else selectedFruitIds[#selectedFruitIds + 1] = labelToId[label] end
			end
...
featureToggle(gameTab, {
	...
	Callback = function(enabled)
		...
		task.spawn(function()
			...
			local ok_tree, res_tree = pcall(function() return require(ReplicatedStorage.Source.Features.Upgrades.UpgradeTree) end)
...
pcall(function()
	...
	if RecipesModule then
		...
		for _, recipe in ipairs(all) do if unlocked[recipe.id] then recipeIdsList[#recipeIdsList + 1] = recipe.id end end
	...
	-- Cache the crafting remote instead of re-fetching every call
	...
	local function getCraftingRemote()
		...
		if ok and remote then craftingRemoteCache = remote end
			...
			indexThread = task.spawn(function()
				...
				local function getSortedCategoriesByPriority()
					...
					for _, catId in ipairs(CATEGORY_IDS) do
						...
						if #missing > 0 then cats[#cats + 1] = { id=catId, easiestEffectiveOdds=getEffectiveOdds(missing[1], catId) } end
					end
...
pcall(function()
	...
	if boostKinds then for _, kind in ipairs(boostKinds) do sortedBoostKinds[#sortedBoostKinds + 1] = kind end table.sort(sortedBoostKinds) end
