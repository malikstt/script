	...
	task.spawn(function()
		...
		local ok, err = pcall(function()
			...
			task.spawn(function()
				...
				until ok1 and ok2
				-- Properly initialize upgrade service
				pcall(function()
	...
	task.spawn(function()
		while true do
			...
			pcall(function()
				...
				local currentMode = rayfieldLibrary.Flags.StackMode and rayfieldLibrary.Flags.StackMode.CurrentOption[1] or "Normal"
				...
				elseif currentMode == "Smart" then
					-- Smart order: galaxy pauses first, then void, then diamond, then golden
					-- If galaxy hasn't paused yet, keep all others running even if ready
					...
					-- If galaxy is in watch but not yet paused, unpause everything else
					if galaxyInWatch and not galaxyPaused then
						...
						-- Galaxy is paused (or not in watch), now sequentially pause void → diamond → golden
						-- Find the first in smartOrder that is in toWatch, paused, and pause next
						...
						for _, dice in ipairs(smartOrder) do
							...
							if inWatch then
								if paused[dice] then
									...
									-- This is the next one to pause — only pause if previous in order is already paused (or not in watch)
						...
						-- Check if all are paused → release
		...
		Callback = function(enabled)
			...
			task.spawn(function()
				...
				while true do
					...
					pcall(function()
						-- If saved position is set, teleport there instead
