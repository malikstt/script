task.spawn(function()
    ...
    task.spawn(function()
        ...
        _0x8c1d4a:CreateDropdown({
local farmingTab = _0x4f2a8c_window:CreateTab("Farming", 138602335586757)

do
    local rollRemote = game:GetService("ReplicatedStorage"):WaitForChild("Packages"):WaitForChild("_Index"):WaitForChild("leifstout_networker@0.3.1"):WaitForChild("networker"):WaitForChild("_remotes"):WaitForChild("RollService"):WaitForChild("RemoteFunction")
    local rollSlice = require(_0x3f7a2b:WaitForChild("Source"):WaitForChild("Features"):WaitForChild("Roll"):WaitForChild("RollSlice"))
                        ...
                        for _, folder in ipairs({"Loot", "Debris"}) do
                            ...
                            if container then
                                for _, item in ipairs(container:GetChildren()) do
                                    ...
                                    if id then
                                        pcall(function() lootRemote:InvokeServer("requestCollect", id) end)
                                    end
                                end
                            end
                        end
...
do
    local FruitsModule = require(_0x3f7a2b:WaitForChild("Source"):WaitForChild("Game"):WaitForChild("Items"):WaitForChild("Fruits"))
    ...
    local selectedFruitSlimeMode = "Best"
    ...
    local function getFruitTargetSlimes()
        local inv = _0x7b3f5a:get("inventory") or {}
        local equipped = _0x7b3f5a:get("equipped") or {}
        if selectedFruitSlimeMode == "Best" then
            local best = _0x2f8c4a()
            if best then return {{key = best, data = inv[best]}} end
            return {}
        else
            local result = {}
            for _, key in pairs(equipped) do
                local data = inv[key]
    ...
    local function doFruitFeed()
        local targets = getFruitTargetSlimes()
        ...
        for _, entry in ipairs(targets) do
            for _, fruitId in ipairs(fruitsToFeed) do
                if not slimeHasFruit(entry.data, fruitId) then
                    pcall(function() _0x9c3a2e:InvokeServer("requestUseFruit", fruitId, entry.key) end)
                    task.wait(0.1)
                end
            end
        end
    ...
    farmingTab:CreateSection("Fruits")
    farmingTab:CreateToggle({
        Name = "Auto Feed Fruits",
        CurrentValue = false,
        Flag = "FarmingAutoFruits",
        Callback = function(val)
            if val then
                if autoFruitConnection then autoFruitConnection:Disconnect() end
    ...
    farmingTab:CreateDropdown({
        ...
        CurrentOption = {"Best"},
        MultipleOptions = false,
        Flag = "FarmingFruitSlimeMode",
        Callback = function(opt)
            selectedFruitSlimeMode = type(opt) == "table" and opt[1] or opt
        end,
    })
    farmingTab:CreateDropdown({
        Name = "Fruits to Use",
                ...
                if bestUid then
                    ...
                    for itemId, count in pairs(items) do
                        if type(count) == "number" and count > 0 then
                            _0x8a1d6f:fetch("requestUseFood", itemId, bestUid, count)
                            task.wait(0.3)
                        end
                    end
                end
    ...
    farmingTab:CreateToggle({
        ...
        Callback = function(val)
            if val then
                task.spawn(function()
                    while _0x2c5d8f.Flags.FarmingStayInBestZone and _0x2c5d8f.Flags.FarmingStayInBestZone.CurrentValue do
                        local target = _0x2c5d8f.Flags.FarmingZoneTarget.CurrentOption[1]
                        if target == "Best Unlocked" then
                            for z = 33, 1, -1 do
                                if not (_0x2c5d8f.Flags.FarmingStayInBestZone and _0x2c5d8f.Flags.FarmingStayInBestZone.CurrentValue) then break end
                                _0x2a7e4c:InvokeServer("requestTeleportZone", z)
                                task.wait(1)
                                if (_0x7b3f5a:get("zone") or 1) == z then break end
                            end
                        else
                            local zoneNum = tonumber(target:match("Zone (%d+)"))
                            if zoneNum then _0x2a7e4c:InvokeServer("requestTeleportZone", zoneNum) end
                        end
                        task.wait(10)
                    end
                end)
...
do
    ...
    local luckLabel = farmingTab:CreateLabel("Total Stacked: x0")
    ...
    task.spawn(function()
        while true do
            ...
            for _, dice in ipairs(DICE_LIST) do
                ...
                if rolls <= 1 then
                    local mult = 0
                    pcall(function() mult = SpecialRollUtils.getLuckMultiplier(dice, upgrades) or 0 end)
                    total = total + mult
                end
            end
            pcall(function() luckLabel:Set("Total Stacked: x" .. string.format("%.1f", total)) end)
        end
    end)
        ...
        while true do
            ...
            for _, dice in ipairs(DICE_LIST) do
                if diceSelected[dice] then
                    local ok = false
                    pcall(function() ok = SpecialRollUtils.isUnlocked(dice, upgrades) end)
            ...
            if allReady then
                ...
                _0x2c5d8f:Notify({Title="Dice Stack", Content="All stacked — releasing now!", Duration=3})
                task.wait(2)
            end
        end
    end)
    ...
    farmingTab:CreateDropdown({
        ...
        CurrentOption = {"All"},
        MultipleOptions = true,
        Flag = "FarmingDiceTypes",
        Callback = function(choices)
            for _, d in ipairs(DICE_LIST) do diceSelected[d] = false end
            for _, ch in ipairs(choices) do
                if ch == "All" then
                    for _, d in ipairs(DICE_LIST) do diceSelected[d] = true end
                    break
                else
                    diceSelected[ch:lower()] = true
                end
            end
        end,
    })
