task.spawn(function()
    ...
    task.spawn(function()
        ...
        -- ========== SERVICES & MODULES (ADDENDUM) ==========
        local SettingsServiceClient = require(source.Features.Settings.SettingsServiceClient)
        local RollSlice2 = require(source.Features.Roll.RollSlice)
        local Slimes2 = require(source.Game.Items.Slimes)
        local FruitsModule2 = require(source.Game.Items.Fruits)
        ...
        local DICE2 = {"golden", "diamond", "void", "galaxy"}

        local indexRunning2 = false
        local indexThread2 = nil
        local luckPollThread2 = nil
        local selectedCategoryOpt2 = nil
        local progressLabels2 = {}
        local luckValueLocal2 = 1

        local selectedDice2 = {golden=true, diamond=true, void=true, galaxy=true}
        local stackActive2 = false
        local pausedDice2 = {golden=false, diamond=false, void=false, galaxy=false}

        local autoFeedEnabled2 = false
        local selectedFruitIds2 = {"ANY"}
        local selectedSlimeMode2 = "Best"
        local feedConnection2 = nil
        ...
        local function applyLuckSettings2()
            setLuck2(1)
            task.wait(0.3)
            setLuckEnabled2(true)
            task.wait(0.3)
        end

        local function applyLuckForTarget2(effectiveOdds)
            setLuck2(calcOptimalLuck2(effectiveOdds))
        end
        ...
        local function getEffectiveOdds2(slime, catId)
            local m = MUTATION_ODDS2[catId]
            if m then return slime.rollOdds * m end
            return slime.rollOdds
        end

        local function getUnlocked2(catId)
            local data = dataServiceClient:get("index") or {}
            return ((data.categories or {})[catId] or {}).unlocked or {}
        end

        local function getTotalSlimes2()
            return #Slimes2.getSortedSlimes()
        end

        local function getUnlockedCount2(catId)
            local count = 0
            for _, v in pairs(getUnlocked2(catId)) do
                if v == true then count += 1 end
            end
            return count
        end

        local function getMissingSlimes2(catId)
            local unlocked = getUnlocked2(catId)
            local missing = {}
            for _, slime in ipairs(Slimes2.getSortedSlimes()) do
                if not unlocked[slime.id] then table.insert(missing, slime) end
            end
            table.sort(missing, function(a, b)
                return getEffectiveOdds2(a, catId) > getEffectiveOdds2(b, catId)
            end)
            return missing
        end

        local function getSortedCategoriesByPriority2()
            local cats = {}
            for _, catId in ipairs(CATEGORY_IDS2) do
                local missing = getMissingSlimes2(catId)
                if #missing > 0 then
                    table.insert(cats, {
                        id = catId,
                        easiestEffectiveOdds = getEffectiveOdds2(missing[1], catId),
                    })
                end
            end
        ...
        local function getCatIdFromOption2(option)
            for _, catId in ipairs(CATEGORY_IDS2) do
        ...
        local function getOwnedFruitIds2()
            local items = dataServiceClient:get("items") or {}
            local owned = {}
            for _, f in ipairs(ALL_FRUITS2) do
                if (items[f.id] or 0) > 0 then owned[f.id] = true end
            end
            return owned
        end

        local function slimeHasFruit2(slimeData, fruitId)
            if type(slimeData) ~= "table" then return false end
            local fruitDef = FruitsModule2.getFruit(fruitId)
        ...
        local function getBestSlimeEntry2()
            ...
            local rarestMut = rarest.slimeData.mutations or {}
            local equipped  = dataServiceClient:get("equipped") or {}
            ...
            for _, slimeKey in pairs(equipped) do
                if type(slimeKey) == "string" and slimeKey:sub(1,1) == "." then
                    ...
                    if type(data) == "table" and data.id == rarestId then
                        local match = true
                        for k, v in pairs(rarestMut) do
                            if not data.mutations or data.mutations[k] ~= v then match = false; break end
                        end
        ...
        local function getTargetSlimes2()
            if selectedSlimeMode2 == "Best" then
                local key, data = getBestSlimeEntry2()
        ...
        local function resolveFruitList2()
            local owned = getOwnedFruitIds2()
            if selectedFruitIds2[1] == "ANY" then
                local result = {}
                for _, f in ipairs(ALL_FRUITS2) do
                ...
                local result = {}
                for _, fid in ipairs(selectedFruitIds2) do
        ...
        local function doFeed2()
            local targets = getTargetSlimes2()
            local fruits  = resolveFruitList2()
            if #targets == 0 or #fruits == 0 then return end
            for _, entry in ipairs(targets) do
                for _, fruitId in ipairs(fruits) do
                    if not slimeHasFruit2(entry.data, fruitId) then
                        networkerInventory2:fetch("requestUseFruit", fruitId, entry.key)
                    end
                end
            end
        end
        ...
        IndexTab:CreateToggle({
            ...
            Callback = function(value)
                if value then
                    indexRunning2 = true
                    indexThread2 = task.spawn(function()
                        applyLuckSettings2()
                        luckPollThread2 = task.spawn(function()
                            while indexRunning2 do
                                if indexLabels2.lLuck then
                                    indexLabels2.lLuck:Set("🍀 Luck Override: x" .. tostring(luckValueLocal2))
                                end
                                task.wait(1)
                            end
                        end)
                        ...
                        if selectedCategoryOpt2 == nil or selectedCategoryOpt2 == "🎲 All (Recommended)" then
                            while indexRunning2 do
                                local sorted = getSortedCategoriesByPriority2()
                                if #sorted == 0 then
                                    if indexLabels2.lCategory then indexLabels2.lCategory:Set("📂 ✅ All Complete!") end
                                    if indexLabels2.lTarget   then indexLabels2.lTarget:Set("🎯 Target: —") end
                                    if indexLabels2.lOdds     then indexLabels2.lOdds:Set("🎲 Odds: —") end
                                    indexRunning2 = false
                                    break
                                end
                                local completed = runCategory2(sorted[1].id, mode, indexLabels2)
                                if not completed then break end
                            end
                        else
                            local catId = getCatIdFromOption2(selectedCategoryOpt2)
                            if catId then
                                runCategory2(catId, mode, indexLabels2)
                                if indexRunning2 then
                                    if indexLabels2.lCategory then indexLabels2.lCategory:Set("📂 ✅ Complete!") end
                                    if indexLabels2.lTarget   then indexLabels2.lTarget:Set("🎯 Target: —") end
                                    if indexLabels2.lOdds     then indexLabels2.lOdds:Set("🎲 Odds: —") end
                                end
                            end
                        ...
                        if luckPollThread2 then task.cancel(luckPollThread2) end
                        setLuckEnabled2(false)
                    end)
                else
                    indexRunning2 = false
                    if indexThread2    then task.cancel(indexThread2) end
                    if luckPollThread2 then task.cancel(luckPollThread2) end
                    setLuckEnabled2(false)
                    if indexLabels2.lTarget   then indexLabels2.lTarget:Set("🎯 Target: —") end
                    if indexLabels2.lOdds     then indexLabels2.lOdds:Set("🎲 Odds: —") end
                    if indexLabels2.lLuck     then indexLabels2.lLuck:Set("🍀 Luck: —") end
                    if indexLabels2.lCategory then indexLabels2.lCategory:Set("📂 Category: —") end
                end
            end,
        })
        ...
        local categoryOptions2 = {"🎲 All (Recommended)"}
        for _, catId in ipairs(CATEGORY_IDS2) do
            local missing = getMissingSlimes2(catId)
            local label   = catId:sub(1,1):upper() .. catId:sub(2)
            if #missing == 0 then
                table.insert(categoryOptions2, "✅ " .. label .. " (Complete)")
            else
                local effOdds = getEffectiveOdds2(missing[1], catId)
                table.insert(categoryOptions2, string.format("%s (%d left | %s)", label, #missing, formatOdds2(effOdds)))
            end
        end
        selectedCategoryOpt2 = categoryOptions2[1]

        IndexTab:CreateDropdown({
            Name = "Category",
            Options = categoryOptions2,
            CurrentOption = {categoryOptions2[1]},
            MultipleOptions = false,
            Flag = "IndexCategory",
            Callback = function(option)
                selectedCategoryOpt2 = type(option) == "table" and option[1] or option
            end,
        })
        ...
        indexLabels2.lTarget   = IndexTab:CreateLabel("🎯 Target: —")
        indexLabels2.lOdds     = IndexTab:CreateLabel("🎲 Odds: —")
        indexLabels2.lLuck     = IndexTab:CreateLabel("🍀 Luck: —")
        indexLabels2.lCategory = IndexTab:CreateLabel("📂 Category: —")

        IndexTab:CreateSection("Index Progress")
        local totalSlimes2 = getTotalSlimes2()
        for _, catId in ipairs(CATEGORY_IDS2) do
            local label = catId:sub(1,1):upper() .. catId:sub(2)
            progressLabels2[catId] = IndexTab:CreateLabel(string.format("📊 %s: %d / %d", label, getUnlockedCount2(catId), totalSlimes2))
        end

        task.spawn(function()
            while true do
                task.wait(2)
                local t = getTotalSlimes2()
                for _, catId in ipairs(CATEGORY_IDS2) do
                    if progressLabels2[catId] then
                        local label = catId:sub(1,1):upper() .. catId:sub(2)
                        progressLabels2[catId]:Set(string.format("📊 %s: %d / %d", label, getUnlockedCount2(catId), t))
                    end
                end
            end
        end)
        ...
        DiceTab:CreateToggle({
            ...
            Callback = function(v)
                stackActive2 = v
                if not v then
                    for _, dice in ipairs(DICE2) do
                        if pausedDice2[dice] then
                            networkerRoll2:fetch("requestSetSpecialRollPaused", dice, false)
                            pausedDice2[dice] = false
                        end
                    end
                end
            end,
        ...
        DiceTab:CreateSection("Dice to Stack")
        DiceTab:CreateDropdown({
            Name = "Select Dice",
            ...
            Callback = function(choices)
                for _, dice in ipairs(DICE2) do selectedDice2[dice] = false end
                for _, choice in ipairs(choices) do
                    if choice == "All" then
                        for _, dice in ipairs(DICE2) do selectedDice2[dice] = true end
                        break
                    else
                        selectedDice2[choice:lower()] = true
                    end
                end
            end,
        })

        DiceTab:CreateSection("Total Stacked Luck")
        local DiceLuckLabel2 = DiceTab:CreateLabel("Total Stacked: x0")
        ...
        task.spawn(function()
            while true do
                ...
                local totalStacked2 = 0
                for _, dice in ipairs(DICE2) do
                    local prog  = progression[dice]
                    local rolls = prog and prog.rollsUntilNext or math.huge
                    if rolls <= 1 then
                        local ok, mult = pcall(SpecialRollUtils2.getLuckMultiplier, dice, upgrades)
                        if ok then totalStacked2 += (mult or 0) end
                    end
                end
                DiceLuckLabel2:Set("Total Stacked: x" .. string.format("%.1f", totalStacked2))

                if not stackActive2 then continue end

                local toWatch = {}
                for _, dice in ipairs(DICE2) do
                    if selectedDice2[dice] then
                        local ok, unlocked = pcall(SpecialRollUtils2.isUnlocked, dice, upgrades)
                ...
                for _, dice in ipairs(toWatch) do
                    ...
                    local rolls = prog and prog.rollsUntilNext or math.huge
                    if rolls <= 1 then
                        if not pausedDice2[dice] then
                            networkerRoll2:fetch("requestSetSpecialRollPaused", dice, true)
                            pausedDice2[dice] = true
                        end
                    else
                        allReady = false
                    end
                ...
                if allReady then
                    ...
                    rayfield:Notify({Title = "Unleashed!", Content = "All stacked — releasing now.", Duration = 3})
                    task.wait(2)
                end
            end
        end)
        ...
        FruitTab:CreateToggle({
            ...
            Flag = "AutoFeedToggle",
            Callback = function(value)
                autoFeedEnabled2 = value
                if value then
                    if feedConnection2 then feedConnection2:Disconnect() end
                    feedConnection2 = game:GetService("RunService").Heartbeat:Connect(function()
                        if autoFeedEnabled2 then doFeed2() end
                    end)
                else
                    if feedConnection2 then feedConnection2:Disconnect(); feedConnection2 = nil end
                end
            end,
        })
        ...
        FruitTab:CreateDropdown({
            ...
            Callback = function(option)
                selectedSlimeMode2 = type(option) == "table" and option[1] or option
            end,
        })

        local fruitOptions2 = {"Any"}
        local labelToId2 = {}
        for _, f in ipairs(ALL_FRUITS2) do
            table.insert(fruitOptions2, f.powerName)
            labelToId2[f.powerName] = f.id
        end

        FruitTab:CreateDropdown({
            Name = "Fruits to Feed",
            Options = fruitOptions2,
            CurrentOption = {"Any"},
            MultipleOptions = true,
            Flag = "FruitDropdown",
            Callback = function(options)
                ...
                for _, label in ipairs(picked) do
                    if label == "Any" then selectedFruitIds2 = {"ANY"}; return
                    else table.insert(selectedFruitIds2, labelToId2[label]) end
                end
                if #selectedFruitIds2 == 0 then selectedFruitIds2 = {"ANY"} end
            end,
        })

        print("Cactus Hub addendum loaded!")
