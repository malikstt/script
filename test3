...
frameCount = 0
lastTime = now
pcall(function() statusLabel:Set("FPS: " .. tostring(fpsValue) .. " / PING: " .. tostring(pingValue) .. "ms") end)
end
end)
end)
task.spawn(function()
...
local packagesOk, packagesResult = pcall(function() return ReplicatedStorage:WaitForChild("Packages", 20) end)
if not packagesOk or not packagesResult then error("Packages not found after 20s") end
packages = packagesResult
local indexFolder
local indexOk = pcall(function() indexFolder = packages:WaitForChild("_Index", 15) end)
if not indexOk or not indexFolder then error("_Index folder not found") end
local networkerPkg
local nPkgOk = pcall(function() networkerPkg = indexFolder:WaitForChild("leifstout_networker@0.3.1", 15) end)
if not nPkgOk or not networkerPkg then error("networker package not found") end
local networkerModule
local nModOk = pcall(function() networkerModule = networkerPkg:WaitForChild("networker", 15) end)
if not nModOk or not networkerModule then error("networker module not found") end
local remotesFolder
local remOk = pcall(function() remotesFolder = networkerModule:WaitForChild("_remotes", 15) end)
if not remOk or not remotesFolder then error("_remotes not found") end
local dsOk, dsErr = pcall(function()
dataServiceClient = require(packages.DataService).client
dataServiceClient:waitForData()
end)
if not dsOk then error("DataService failed: " .. tostring(dsErr)) end
...
if not nOk then error("Networker failed: " .. tostring(nErr)) end
...
return folder:FindFirstChild("RemoteFunction") or folder:WaitForChild("RemoteFunction", 10)
end)
if not ok3 or not rf then
...
local sfOk, sfErr = pcall(function() sourceFolder = ReplicatedStorage:WaitForChild("Source", 30) end)
if not sfOk or not sourceFolder then error("Source folder not found: " .. tostring(sfErr)) end
local function safeRequire(path, name)
local ok2, result = pcall(function() return require(path) end)
if not ok2 then
Logger:warn("CactusHub", "ModuleLoad", "Failed to require " .. name .. ": " .. tostring(result))
return nil
end
return result
end
upgradeTreeModule  = safeRequire(sourceFolder.Features.Upgrades.UpgradeTree, "UpgradeTree")
indexRewardsModule = safeRequire(sourceFolder.Features.Index.IndexRewards, "IndexRewards")
boostServiceUtils  = safeRequire(sourceFolder.Features.Boosts.BoostServiceUtils, "BoostServiceUtils")
specialDiceUtils   = safeRequire(sourceFolder.Features.SpecialDice.SpecialDiceServiceUtils, "SpecialDiceServiceUtils")
rollSliceModule    = safeRequire(sourceFolder.Features.Roll.RollSlice, "RollSlice")
slimesModule       = safeRequire(sourceFolder.Game.Items.Slimes, "Slimes")
mutationsModule    = safeRequire(sourceFolder.Features.Mutations.Mutations, "Mutations")
FruitsModule       = safeRequire(sourceFolder.Game.Items.Fruits, "Fruits")
SpecialRollUtils   = safeRequire(sourceFolder.Features.Roll.SpecialRollUtils, "SpecialRollUtils")
ZonesModule        = safeRequire(sourceFolder.Game.Items.Zones, "Zones")
if boostServiceUtils then
pcall(function() boostKinds = boostServiceUtils.getKinds() end)
end
if specialDiceUtils then
pcall(function()
...
pcall(function()
SettingsState         = safeRequire(sourceFolder.Features.Settings.SettingsState, "SettingsState")
SettingsServiceClient = safeRequire(sourceFolder.Features.Settings.SettingsServiceClient, "SettingsServiceClient")
if SettingsState and SettingsServiceClient then
SettingsState.init()
...
pcall(function() RecipesModule = safeRequire(sourceFolder.Features.Crafting.Recipes, "Recipes") end)
task.spawn(function()
...
pcall(function()
rayfieldLibrary:Notify({ Title = "CactusHub — Load Warning", Content = "Some modules failed: " .. tostring(loadErr):sub(1, 120) .. "\nSome features may not work.", Duration = 8 })
end)
end
end)
...
task.spawn(function()
repeat task.wait(0.5) until modulesLoaded and Networker and SettingsServiceClient
pcall(function()
settingsClientRef = {}
...
if mutOdds then return (slime.rollOdds or 0) * mutOdds end
return slime.rollOdds or 0
end
...
local ok2, sortedSlimes = pcall(function() return slimesModule.getSortedSlimes() end)
if not ok2 then return {} end
local unlocked = getUnlockedIndex(catId)
local missing = {}
for _, slime in ipairs(sortedSlimes) do
...
local ok2, stats = pcall(function() return dataServiceClient:get("stats") or {} end)
...
local ok3, inventory = pcall(function() return dataServiceClient:get("inventory") or {} end)
...
local priority = "Closest"
pcall(function()
priority = rayfieldLibrary.Flags.CombatTargetPriority and rayfieldLibrary.Flags.CombatTargetPriority.CurrentOption[1] or "Closest"
end)
...
currentTarget = nil
local safeCenter = getSafePosition(CFrame.new(boundary.center), boundary)
pcall(function() char:PivotTo(CFrame.new(safeCenter)) end)
task.wait(0.5)
return
end
...
local rollTime = 0.5
if rollSliceModule then
pcall(function() rollTime = rollSliceModule.rollTime() or 0.5 end)
end
task.wait(rollTime)
end
end)
...
local upgrades = {}
local progression = {}
pcall(function() upgrades = dataServiceClient:get("upgrades") or {} end)
pcall(function() progression = dataServiceClient:get("specialRollProgression") or {} end)
...
pcall(function() DiceLuckLabel:Set("Total Stacked: x" .. string.format("%.1f", totalStacked)) end)
...
pcall(function()
currentMode = (rayfieldLibrary.Flags.StackMode and rayfieldLibrary.Flags.StackMode.CurrentOption and rayfieldLibrary.Flags.StackMode.CurrentOption[1]) or "Normal"
end)
...
local totalZones = 40
if ZonesModule then
pcall(function() totalZones = ZonesModule.getMaxZone() or 40 end)
end
for i = 1, totalZones do
local zone = nil
if ZonesModule then pcall(function() zone = ZonesModule.getZone(i) end) end
...
pcall(function()
targetOption = rayfieldLibrary.Flags.FarmingZoneTarget and rayfieldLibrary.Flags.FarmingZoneTarget.CurrentOption[1] or "Best Unlocked"
end)
local currentZone = 1
pcall(function() currentZone = dataServiceClient and (dataServiceClient:get("zone") or 1) or 1 end)
local targetZone = nil
if targetOption == "Best Unlocked" then
pcall(function() targetZone = dataServiceClient and (dataServiceClient:get("maxZone") or 1) or 1 end)
else
...
local selected = {"All"}
pcall(function()
selected = rayfieldLibrary.Flags.MachineUnlockTarget and rayfieldLibrary.Flags.MachineUnlockTarget.CurrentOption or {"All"}
end)
local selSet = {}
...
local ok2, items = pcall(function() return dataServiceClient:get("items") or {} end)
if not ok2 then return {} end
local owned = {}
...
local fruitDef = nil
pcall(function() fruitDef = FruitsModule.getFruit(fruitId) end)
...
local ok3, equipped = pcall(function() return dataServiceClient:get("equipped") or {} end)
if not ok3 then return nil, nil end
local ok4, inv = pcall(function() return dataServiceClient:get("inventory") or {} end)
if not ok4 then return nil, nil end
...
local ok2, equipped = pcall(function() return dataServiceClient:get("equipped") or {} end)
if not ok2 then return {} end
local result = {}
for _, slimeKey in ipairs(equipped) do
if type(slimeKey) == "string" and slimeKey:sub(1,1) == "." then
local ok3, inv = pcall(function() return dataServiceClient:get("inventory") or {} end)
if ok3 then
local data = inv[slimeKey]
...
local fruitsSource = {}
if FruitsModule then
pcall(function() fruitsSource = FruitsModule.getSortedFruits() or {} end)
end
for _, f in ipairs(fruitsSource) do
...
local ok2, inventory = pcall(function() return dataServiceClient:get("inventory") or {} end)
local ok3, equipped  = pcall(function() return dataServiceClient:get("equipped") or {} end)
if not ok2 or not ok3 then return end
local teamSet = {}
for _, uid in ipairs(equipped) do teamSet[uid] = true end
local targetOption = "Best Slime"
pcall(function()
targetOption = rayfieldLibrary.Flags.FarmingTransferTarget and rayfieldLibrary.Flags.FarmingTransferTarget.CurrentOption and rayfieldLibrary.Flags.FarmingTransferTarget.CurrentOption[1] or "Best Slime"
end)
local sourceOption = "Unequipped With XP"
pcall(function()
sourceOption = rayfieldLibrary.Flags.FarmingTransferSource and rayfieldLibrary.Flags.FarmingTransferSource.CurrentOption and rayfieldLibrary.Flags.FarmingTransferSource.CurrentOption[1] or "Unequipped With XP"
end)
local targets = {}
...
local id = nil
pcall(function() id = item:GetAttribute("uniqueId") or item:GetAttribute("id") or item.Name end)
...
local fruits = {}
if FruitsModule then
pcall(function() fruits = (FruitsModule.getSortedFruits and FruitsModule.getSortedFruits()) or {} end)
end
...
local minZone      = 0
pcall(function() minZone = tonumber(rayfieldLibrary.Flags.GameMinZoneRebirth and rayfieldLibrary.Flags.GameMinZoneRebirth.CurrentValue) or 0 end)
...
pcall(function()
local upgradeMode = {"All"}
pcall(function()
upgradeMode = rayfieldLibrary.Flags.GameUpgradeMode and rayfieldLibrary.Flags.GameUpgradeMode.CurrentOption or {"All"}
end)
local modeSet = {}
...
local ok2, data = pcall(function() return MutationsModule.get(mutId) end)
return (ok2 and data and data.value) or 0
end
...
if dataServiceClient then
local ok2, inv = pcall(function() return dataServiceClient:get("inventory") or {} end)
if ok2 then
...
local ok2, recipe = pcall(function() return RecipesModule.getRecipe(recipeId) end)
if not ok2 or not recipe then return nil end
...
return ReplicatedStorage:WaitForChild("Packages",15):WaitForChild("_Index",15):WaitForChild("leifstout_networker@0.3.1",15):WaitForChild("networker",15):WaitForChild("_remotes",15):WaitForChild("CraftingService",10):WaitForChild("RemoteFunction",10)
end
local function doCraftAll(amount)
local ok2, craftRemote = pcall(getCraftingRemote)
if not ok2 or not craftRemote then return {} end
local results = {}
for _, recipeId in ipairs(craftingState.selectedRecipeIds) do
local args = buildCraftArgsForRecipe(recipeId, amount)
if args then
local ok3, result = pcall(function() return craftRemote:InvokeServer(table.unpack(args)) end)
...
local ok2, recipe = pcall(function() return RecipesModule.getRecipe(recipeId) end)
if not ok2 or not recipe then return 0 end
...
if not ufoClient then pcall(function() ufoPhaseLabel:Set("🛸  Phase: Module not loaded") end) return end
...
local ok3, secs = pcall(function() return math.max(0, math.round(state.nextEventStartTime - workspace:GetServerTimeNow())) end)
if ok3 then nextEvent = string.format("%02d:%02d", math.floor(secs/60), secs%60) end
end
local phaseIcon = "⚪"
...
pcall(function() ufoPhaseLabel:Set("🛸  Phase: " .. phaseIcon .. " " .. state.phase:upper()) end)
pcall(function() ufoZoneIdLabel:Set("📍  Zone ID: " .. (state.zoneId and tostring(state.zoneId) or "None")) end)
pcall(function() ufoZoneNameLabel:Set("🗺️  Zone Name: " .. zoneName) end)
pcall(function() ufoNextLabel:Set("⏳  Next Event: " .. nextEvent) end)
pcall(function() ufoGoldenLabel:Set("⭐  Golden UFO: " .. (isGolden and "Yes ✅" or "No ❌")) end)
end
...
if dataServiceClient then pcall(function() currentZone = dataServiceClient:get("zone") end) end
if currentZone ~= state.zoneId and ufoZonesRemote then
...
pcall(function() setLuck(1) end) task.wait(0.3)
pcall(function() setLuckEnabled(true) end) task.wait(0.3)
...
local mode = "🌱 Easiest First"
pcall(function()
local modeFlag = rayfieldLibrary.Flags and rayfieldLibrary.Flags.IndexRollMode
mode = modeFlag and (type(modeFlag.CurrentOption)=="table" and modeFlag.CurrentOption[1] or modeFlag.CurrentOption) or "🌱 Easiest First"
end)
...
if target.id ~= lastTargetId then lastTargetId = target.id pcall(function() setLuck(calcOptimalLuck(effOdds)) end) end
if labels then
...
if networkerRoll then
pcall(function() networkerRoll:fetch("requestRoll") end)
end
local rollTime = 0.5
if rollSliceModule then pcall(function() rollTime = rollSliceModule.rollTime() or 0.5 end) end
task.wait(rollTime)
...
local slime = nil
if slimesModule then pcall(function() slime = slimesModule.getSlime(id) end) end
...
pcall(function() setLuckEnabled(false) end)
end)
else
indexRunning = false
...
local ok2, indexData = pcall(function() return dataServiceClient:get("index") end)
if not ok2 or not indexData or not indexData.categories then return end
...
local ok2, boosts = pcall(function() return dataServiceClient:get("boosts") or {} end)
if not ok2 then return end
local selectedPotions = {}
pcall(function()
selectedPotions = rayfieldLibrary.Flags.MiscPotionTypes and rayfieldLibrary.Flags.MiscPotionTypes.CurrentOption or {}
end)
local ok3, serverTime = pcall(function() return workspace:GetServerTimeNow() end)
...
local ok4, remaining = pcall(boostServiceUtils.getTimeRemaining, boostData, serverTime)
if ok4 and remaining <= 0 then
...
local ok2, queue = pcall(function() return dataServiceClient:get("specialDiceQueue") or {} end)
if ok2 and queue[1] then task.wait(1) return end
local ok3, items = pcall(function() return dataServiceClient:get("items") or {} end)
if not ok3 then return end
local selectedDiceItems = {}
pcall(function()
selectedDiceItems = rayfieldLibrary.Flags.MiscDiceTypes and rayfieldLibrary.Flags.MiscDiceTypes.CurrentOption or {}
end)
...
pcall(function() webhookEnabled = rayfieldLibrary.Flags.WebhookEnabled.CurrentValue end)
if not webhookEnabled then rayfieldLibrary:Notify({Title="Webhook",Content="Please enable Webhook first.",Duration=4}) return end
local userId = ""
pcall(function() userId = rayfieldLibrary.Flags.WebhookUserID.CurrentValue end)
...
local ok2, indexData = pcall(function() return dataServiceClient:get("index") or {} end)
...
if mutations and mutationsModule then
pcall(function() displayName = mutationsModule.getDisplayName(slimeName, mutations) end)
end
...
local oddsMultiplier = 1
if mutations and mutationsModule then
pcall(function() oddsMultiplier = mutationsModule.getVisualOddsMultiplier(mutations) or 1 end)
end
local statBonus = 1
if mutations and mutationsModule then
pcall(function() statBonus = mutationsModule.getStatBonus(mutations, "damage") or 1 end)
end
...
local totalKills = 0
if dataServiceClient then
pcall(function() totalRolls = (dataServiceClient:get("stats") or {}).rolls or 0 end)
pcall(function() coins = dataServiceClient:get("coins") or 0 end)
pcall(function() totalKills = (dataServiceClient:get("stats") or {}).kills or 0 end)
end
...
local webhookEnabled = false
pcall(function() webhookEnabled = rayfieldLibrary.Flags.WebhookEnabled and rayfieldLibrary.Flags.WebhookEnabled.CurrentValue end)
...
pcall(function()
local ok2, currentRolls = pcall(function() return rollSliceModule.rollResults() end)
if not ok2 or type(currentRolls) ~= "table" or #currentRolls == 0 then return end
...
local sendAll, sendNewOnly, sendMutated = false, false, false
pcall(function()
sendAll     = rayfieldLibrary.Flags.WebhookSendAll and rayfieldLibrary.Flags.WebhookSendAll.CurrentValue
sendNewOnly = rayfieldLibrary.Flags.WebhookSendNew and rayfieldLibrary.Flags.WebhookSendNew.CurrentValue
sendMutated = rayfieldLibrary.Flags.WebhookSendMutated and rayfieldLibrary.Flags.WebhookSendMutated.CurrentValue
end)
local minChanceStr = ""
pcall(function() minChanceStr = rayfieldLibrary.Flags.WebhookMinChance and rayfieldLibrary.Flags.WebhookMinChance.CurrentValue or "" end)
...
local slimeDefinition = nil
if slimesModule then pcall(function() slimeDefinition = slimesModule.getSlime(slimeId) end) end
...
if shouldSend then
local userId = ""
pcall(function() userId = rayfieldLibrary.Flags.WebhookUserID and rayfieldLibrary.Flags.WebhookUserID.CurrentValue or "" end)
...
pcall(function() antiKickEnabled = rayfieldLibrary.Flags.SettingsAntiKick and rayfieldLibrary.Flags.SettingsAntiKick.CurrentValue end)
...
pcall(function() statLabels.sess:Set(string.format("Session: %dh%dm%ds  |  Played: %s  |  Rebirths: %s",sh,sm,ss,fmtTime(timePlayed),fmt(rebirths))) end)
pcall(function() statLabels.rolls1:Set(string.format("Rolls/sec: %.2f  |  Rolls/min: %s  |  Rolls/hr: %s",rps,fmt(rps*60),fmt(rps*3600))) end)
pcall(function() statLabels.rolls2:Set("Session Rolls: "..fmt(sessionRolls).."  |  Lifetime: "..fmt(rolls)) end)
pcall(function() statLabels.coins1:Set("Coins/min: "..fmt(cps*60).."  |  Coins/hr: "..fmt(cps*3600)) end)
pcall(function() statLabels.coins2:Set("Session Coins: "..fmt(sessionCoins).."  |  Total Ever: "..fmt(totalCoins)) end)
pcall(function() statLabels.goop1:Set("Goop/min: "..fmt(gps*60).."  |  Goop/hr: "..fmt(gps*3600)) end)
pcall(function() statLabels.goop2:Set("Session Goop: "..fmt(sessionGoop).."  |  Balance: "..fmt(goop)) end)
pcall(function() statLabels.kills:Set("Session Kills: "..fmt(sessionKills).."  |  Lifetime Kills: "..fmt(kills)) end)
pcall(function() statLabels.best:Set("Best Ever: "..bestName.."  |  Odds: "..bestOdds) end)
pcall(function() statLabels.daily:Set("Best Today Odds: "..dailyStr) end)
pcall(function() statLabels.prog:Set("Zone: "..fmt(zone).."  |  Max Zone: "..fmt(maxZone).."  |  Roll Currency: "..fmt(rollCurrency)) end)
pcall(function() statLabels.idx1:Set("Basic: "..basic.."  |  Big: "..big.."  |  Shiny: "..shiny.."  |  Huge: "..huge.."  |  Inverted: "..inverted) end)
pcall(function() statLabels.inv:Set("Total Slimes: "..fmt(getTotalInventory()).."  |  Species: "..getUniqueSpecies().."  |  Crafting: "..crafting) end)
pcall(function() statLabels.equipped:Set("Equipped: "..getEquippedDisplay()) end)
end)
end
end)
game:GetService("GuiService").ErrorMessageChanged:Connect(function()
local autoRejoinEnabled = false
pcall(function() autoRejoinEnabled = rayfieldLibrary.Flags.SettingsAutoRejoin and rayfieldLibrary.Flags.SettingsAutoRejoin.CurrentValue end)
