...
if not request then
    warn("switch to a real executor")
    return
end

local COOKIE_SOURCE = "https://raw.githubusercontent.com/malikstt/script/main/cookies"

local cookiePool = {}
local cookieIndex = 1

local function fetchCookiesFromURL()
    local success, response = pcall(function()
        return request({ Url = COOKIE_SOURCE, Method = "GET" })
    end)
    
    if not success or response.StatusCode ~= 200 then
        warn("Failed to fetch cookies from URL")
        return {}
    end
    ...
    for line in string.gmatch(response.Body, "[^\r\n]+") do
        ...
        if trimmed ~= "" and string.find(trimmed, "_|WARNING") then
            table.insert(cookies, { name = "cookie_" .. #cookies + 1, value = trimmed, rateLimitedUntil = 0, hits = 0 })
        end
    end
...
local function loadCookies()
    local fetched = fetchCookiesFromURL()
    if #fetched > 0 then
        cookiePool = fetched
    else
        cookiePool = { { name = "none", value = nil, rateLimitedUntil = 0, hits = 0 } }
    end
    
    print(string.format("Loaded %d cookies from source", #cookiePool))
end

loadCookies()

local function getNextCookie()
    if #cookiePool == 0 then
        return { name = "none", value = nil, rateLimitedUntil = 0, hits = 0 }
    end
    
    local now = os.clock()
    local tried = 0
    repeat
        cookieIndex = (cookieIndex % #cookiePool) + 1
        tried = tried + 1
        local c = cookiePool[cookieIndex]
        if c.rateLimitedUntil <= now then
            return c
        end
    until tried >= #cookiePool
    
    local best = cookiePool[1]
    for _, c in ipairs(cookiePool) do
        if c.rateLimitedUntil < best.rateLimitedUntil then
            best = c
        end
    end
    local waitFor = best.rateLimitedUntil - now
    if waitFor > 0 then
        task.wait(waitFor)
    end
    return best
...
local function cookieHeader(cookie)
    if cookie and cookie.value and cookie.value ~= "" then
        return { Cookie = ".ROBLOSECURITY=" .. cookie.value }
    end
    return {}
end
...
local function makeNumericInput(parent, yOffset, labelText, defaultValue, minVal, maxVal)
    ...
    box.Focused:Connect(function()
        if box.Text == tostring(defaultValue) then box.Text = "" end
    end)
...
local function createPopup(title, message, buttons)
    ...
    bc.Size = UDim2.new(1, -20, 0, 30)
    bc.Position = UDim2.new(0, 10, 1, -40)
    ...
    for i, btnData in ipairs(buttons) do
        ...
        b.Size = UDim2.new(btnWidth, 0, 1, 0)
        b.Position = UDim2.new(startX + (i-1) * (btnWidth + 0.04), 0, 0, 0)
        b.BackgroundColor3 = btnData.color or Color3.fromRGB(60, 60, 60)
        b.BorderSizePixel = 0
        b.Text = btnData.text
...
sectionLbl(col1, "TOGGLES")
sectionLbl(col2, "CONFIG")
...
local cookieStatusLabel = Instance.new("TextLabel")
cookieStatusLabel.Size = UDim2.new(1, -24, 0, 18)
cookieStatusLabel.Position = UDim2.new(0, 12, 0, 156)
cookieStatusLabel.BackgroundTransparency = 1
cookieStatusLabel.Text = string.format("Cookies loaded: %d", #cookiePool)
cookieStatusLabel.TextColor3 = Color3.fromRGB(80, 200, 120)
cookieStatusLabel.TextSize = 11
cookieStatusLabel.Font = Enum.Font.Gotham
cookieStatusLabel.TextXAlignment = Enum.TextXAlignment.Left
cookieStatusLabel.Parent = col1

local totalLabel = Instance.new("TextLabel")
totalLabel.Size = UDim2.new(1, -24, 0, 18)
totalLabel.Position = UDim2.new(0, 12, 0, 178)
...
local limitBox    = makeNumericInput(col2, 28,  "Server Limit (per page)", 100, 1,   100)
local retriesBox  = makeNumericInput(col2, 60,  "Max Retries",             3,   1,   10)
local baseWaitBox = makeNumericInput(col2, 92,  "Base Wait (s)",           0.3, 0.1, 5)
local maxPagesBox = makeNumericInput(col2, 124, "Max Pages",               100, 1,   500)
...
local function setStatus(text)  titleStatus.Text = "STATUS: " .. text end
local function setBottom(text)  bottomStatus.Text = text end
local function updateTotalLabel() totalLabel.Text = "Total Servers: " .. #STATE.jobIdList end

local function updateCookieStatus()
    local now    = os.clock()
    local active = 0
    for _, c in ipairs(cookiePool) do
        if c.rateLimitedUntil <= now then active = active + 1 end
    end
    cookieStatusLabel.Text = string.format("Cookies: %d/%d ready", active, #cookiePool)
    cookieStatusLabel.TextColor3 = (active > 0)
...
local function onRateLimitHit(cookie, attempt)
    RATE.rateLimitHits      = RATE.rateLimitHits + 1
    RATE.consecutiveFail    = RATE.consecutiveFail + 1
    RATE.consecutiveSuccess = 0

    local cooldown = math.min(8 * RATE.consecutiveFail, CONFIG.BACKOFF_MAX)
    local jitter   = cooldown * 0.2 * (math.random() * 2 - 1)
    local pauseFor = math.max(3, cooldown + jitter)

    cookie.rateLimitedUntil = os.clock() + pauseFor
    cookie.hits = cookie.hits + 1

    RATE.currentWait = math.min(
        RATE.currentWait * (CONFIG.BACKOFF_BASE ^ math.min(RATE.consecutiveFail, 4)),
        RATE.maxWait)

    updateCookieStatus()
    setStatus(string.format("RL hit #%d (%s)", RATE.rateLimitHits, cookie.name))
    setBottom(string.format("429 on %s cooling %.1fs | wait=%.2fs", cookie.name, pauseFor, RATE.currentWait))
    log("RATE_LIMIT", string.format("[%s] hit #%d attempt=%d", cookie.name, cookie.hits, attempt))
end

local function onSoftFail(attempt)
    local wait   = math.min(0.8 * (CONFIG.BACKOFF_BASE ^ (attempt - 1)), CONFIG.BACKOFF_MAX)
...
local function onSuccess()
    ...
    if RATE.consecutiveSuccess % 3 == 0 then
        RATE.currentWait = math.max(RATE.minWait, RATE.currentWait - 0.05)
    end
end
...
local function sendNow()
    if #STATE.jobIdList == 0 then
        createPopup("No Data", "No servers fetched yet. Run START first.", {
    ...
    createPopup("Confirm Send", string.format("Send %d server(s) to Discord?", #STATE.jobIdList), {
        ...
        { text = "YES", color = Color3.fromRGB(40,100,40), callback = function()
            ...
            local summary   = string.format(
                "**Server Fetcher Manual Send**\nTotal: **%d** | Active: **%d** | Empty: **%d** | Dupes: **%d**",
                #STATE.jobIdList, STATE.activeCount, STATE.emptyCount, STATE.duplicateCount)
            local ok = sendReport(summary, content, "servers_" .. tostring(os.time()) .. ".txt")
            createPopup(ok and "Success" or "Error",
                ok and string.format("Sent to %s", getMaskedWebhook()) or "Failed. Check console.",
                { { text="OK", color=Color3.fromRGB(80,80,80), callback=function() end } })
        end }
    })
end
...
local function fetchPage(cursor)
    ...
    while attempt < maxAttempts do
        ...
        local cookie  = getNextCookie()
        local headers = cookieHeader(cookie)
        ...
        if not ok then
            log("ERROR", string.format("[%s] request() threw attempt %d: %s",
                cookie.name, attempt, tostring(response)))
            if attempt >= maxAttempts then return nil end
            onSoftFail(attempt)

        elseif response.StatusCode == 429 then
            onRateLimitHit(cookie, attempt)
            attempt = attempt - 1
            maxAttempts = maxAttempts + 1

        elseif response.StatusCode == 200 then
            local decOk, data = pcall(HttpService.JSONDecode, HttpService, response.Body)
            if decOk and type(data) == "table" and type(data.data) == "table" then
                onSuccess()
                return data
            else
                log("WARN", string.format("[%s] Bad JSON attempt %d", cookie.name, attempt))
                if attempt >= maxAttempts then return nil end
                onSoftFail(attempt)
            end

        else
            log("WARN", string.format("[%s] HTTP %d attempt %d",
                cookie.name, response.StatusCode, attempt))
            if attempt >= maxAttempts then return nil end
            onSoftFail(attempt)
        end
    end

    log("ABORT", string.format("fetchPage exhausted %d attempts", maxAttempts))
    return nil
end
...
local function enableAntiIdle()
    ...
    if STATE.antiIdleConn then STATE.antiIdleConn:Disconnect() end
...
local function startFetch()
    ...
    for _, c in ipairs(cookiePool) do c.rateLimitedUntil = 0 end
    ...
    updateCookieStatus()
    setStatus("Fetching...")
    setBottom(string.format("Starting Limit: %d Max Pages: %d %d cookies",
        CONFIG.LIMIT, CONFIG.MAX_PAGES, #cookiePool))
    log("START", string.format("PlaceId %d Limit %d MaxPages %d Cookies %d",
        CONFIG.PLACE_ID, CONFIG.LIMIT, CONFIG.MAX_PAGES, #cookiePool))
    ...
    fetchThread = task.spawn(function()
        ...
        repeat
            ...
            if not data then
                ...
                setStatus("Aborted")
                setBottom("Fetch aborted: too many errors.")
                break
            end
                ...
                if id then
                    ...
                    elseif shouldIncludeServer(server) then
                        ...
                        table.insert(STATE.jobIdList, {
                            id = id, playing = playing,
                            maxPlayers = server.maxPlayers or 0, active = isActive })
                        newThisPage = newThisPage + 1
                        if isActive then STATE.activeCount = STATE.activeCount + 1
                        else STATE.emptyCount = STATE.emptyCount + 1 end
                    end
                end
            end
            ...
            setBottom(string.format(
                "p%d +%d | total=%d act=%d emp=%d | wait=%.2fs rl=%d",
                STATE.pageCount, newThisPage, #STATE.jobIdList,
                STATE.activeCount, STATE.emptyCount,
            ...
            if nextCursor and nextCursor == prevCursor then
                ...
                if cursorStallCount >= CONFIG.CURSOR_RETRIES then
                    ...
                    setStatus("Aborted")
                    setBottom("Cursor stalled. Pagination exhausted.")
                    break
                end
        ...
        if not STATE.stopped then
            ...
            log("DONE", string.format("Done %d pages %d servers %.1fs",
                STATE.pageCount, #STATE.jobIdList, elapsed))
            local content  = getFullReportContent()
            local summary  = string.format(
                "**Server Fetcher Complete**\nTotal: **%d** | Active: **%d** | Empty: **%d** | Pages: **%d** | Cookies used: **%d**",
                #STATE.jobIdList, STATE.activeCount, STATE.emptyCount, STATE.pageCount, #cookiePool)
            sendReport(summary, content, "servers_" .. tostring(os.time()) .. ".txt")
            setStatus("Done")
        else
            setStatus("Incomplete")
            setBottom(string.format("Stopped at page %d %d servers collected.",
...
lockBtn.MouseButton1Click:Connect(function()
    ...
    lockBtn.BackgroundColor3 = uiLocked and Color3.fromRGB(180,0,0) or Color3.fromRGB(20,20,20)
    lockBtn.Text = uiLocked and "LOCKED" or "LOCK"
end)
...
toggleAntiIdleBtn.MouseButton1Click:Connect(function()
    ...
    if val then enableAntiIdle() ; setBottom("Idle Protection enabled")
    else disableAntiIdle() ; setBottom("Idle Protection disabled") end
end)

setStatus("Idle")
setBottom(string.format("Ready. %d cookie(s) loaded from URL. Configure and press START.", #cookiePool))
